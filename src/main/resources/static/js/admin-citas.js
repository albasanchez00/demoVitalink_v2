(() => {
    ...
    const btnCerrarModalFooter = $("#btnCerrarModalFooter");
    ...
    q.addEventListener("input", debounce(() => { state.page = 0; cargar(); }, 350));
    estado.addEventListener("change", () => { state.page = 0; cargar(); });
    desde.addEventListener("change", () => { state.page = 0; cargar(); });
    hasta.addEventListener("change", () => { state.page = 0; cargar(); });

    // Limpiar filtros de paciente/médico si se borra el input
    ...
    document.addEventListener("keydown", (e) => {
        if (e.key === "Escape" && modalCita.classList.contains("active")) cerrarModal();
    });
    ...
    async function cargar() {
        ...
        tbody.innerHTML = page.content.map(c => `
            <tr>
                ...
                <td><span class="badge-estado badge-${(c.estado || '').toUpperCase()}">${c.estado ?? "—"}</span></td>
    ...
    async function ver(id) {
        ...
        $("#mc-id").textContent = c.id;
        $("#mc-paciente").textContent = c.pacienteNombre || "—";
        $("#mc-medico").textContent = c.medicoNombre || "—";
        $("#mc-titulo").textContent = c.titulo || "—";
        $("#mc-fecha").textContent = fmtDate(c.fecha);
        $("#mc-hora").textContent = fmtTime(c.hora);

        const estadoEl = $("#mc-estado");
        estadoEl.textContent = c.estado || "—";
        estadoEl.className = "badge-estado " + (c.estado || "");

        const descWrap = $("#mc-desc-wrap");
        if (c.descripcion && c.descripcion.trim()) {
            $("#mc-descripcion").textContent = c.descripcion;
